...
export class LazyImageService {
        ...
        this.observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (src && img.src !== src) {
                        this.loadImageWithRetry(src, img, options.retryCount || 3, options.timeout || 8000);
     ...
     * 批量观察图片元素
     */
    observeAll(images: NodeListOf<HTMLImageElement> | HTMLImageElement[]): void {
     ...
     * 加载图片
     */
    private loadImage(url: string, img: HTMLImageElement, timeout: number): Promise<void> {
