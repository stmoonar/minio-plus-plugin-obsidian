...
export default class MinioPlusPlugin extends Plugin {
    ...
    async handleUploader(evt: ClipboardEvent | DragEvent, editor: Editor): Promise<void> {
        ...
        const previewText = await this.showUploadPreview(editor, startPos, file);

        try {
            const objectName = this.fileProcessor.generateObjectName(file);
            const fileType = this.fileProcessor.getFileType(file);

            await this.uploadService.uploadFile(file, objectName, (progress: UploadProgress) => {
                this.updateUploadProgress(editor, startPos, previewText, progress.percentage);
            });

            const url = this.uploadService.generateAccessUrl(objectName);

            setTimeout(() => {
                const finalText = this.fileProcessor.wrapFileDependingOnType(fileType, url, file.name);
                editor.replaceRange(finalText, startPos, {
                    line: startPos.line,
    ...
    private async showUploadPreview(editor: Editor, startPos: Position, file: File): Promise<string> {
        const fileType = this.fileProcessor.getFileType(file);
    ...
    private updateUploadProgress(
        ...
        const updatedText = originalText.replace(/width: \d+%/, `width: ${percentage}%`);

        editor.replaceRange(updatedText, startPos, {
            line: startPos.line,
            ch: startPos.ch + originalText.length
        });

        if (percentage === 100) {
            const completedText = updatedText.replace('uploading', 'completed');
            editor.replaceRange(completedText, startPos, {
                line: startPos.line,
                ch: startPos.ch + updatedText.length
            });
        }
    }

    validateSettings(): boolean {
        return this.uploadService?.validateUploadConfig().valid ??
